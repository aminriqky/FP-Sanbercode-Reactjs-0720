...
function Content(props) {
  ...
  const handleOpen = () => {
    setOpen(true);
    let res = axios.post(`https://backendexample.sanbersy.com/api/movies/${daftarFilm.id}`, {title: fields.title, description: fields.description, year: fields.year, duration: fields.duration, genre: fields.genre, rating: fields.rating, review: fields.review, image_url: fields.image_url})
    let data = res.data;
    console.log(data);
  };
  ...
  return (
    <>
    {
        daftarFilm !== null && daftarFilm.map((modal)=>{
          return(
            <>
      ...
      }
      {
        daftarFilm !== null && daftarFilm.map((item)=>{
          return(
            ...
            <ButtonBase onClick={handleOpen} onChange={handleFieldChange}>
              <Toolbar>
